...
const MainSearchPrac02 = () => {
  ...
  const searchData3 = async (sendData) => {
    await axios
      ...
      .then((response) => {
        ...
        dispatch(offerInit(response.data)); // 밑 코드 전의 코드
        // handleDispatch(offerInit, response.data);
  ...
  const handleDispatch = async (method, data) => {
    await dispatch(method(data));
  };
  ...
  const handleToResult = () => {
    ...
    // dispatch(searchInit(info2)); // 밑 코드 전의 코드
    // handleDispatch(searchInit, info2);
    // searchData(' ');
    searchData3(info2);
    // let data = SearchData(' ');
